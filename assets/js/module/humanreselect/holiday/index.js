...
var AppHoliday = function() {
	...
	var init = function() {
		...
		$('#regModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				var url = button.data('url')?button.data('url'):KTAppOptions._RF+"/m/humanreselect/form_holiday/add";
				var modal = $(this);
				...
				$.ajax({
					...
					banknumberForm.init(modal.find("form"),modal,id);
					banknumberForm.initRemove();
					setTimeout(function() {
						KTApp.unblock(blockelm);
					}, 3000);
				});
		...
		$('#detailModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				var button = $(event.relatedTarget); 
				var id = button.data('id');
				var url = button.data('url')?button.data('url'):KTAppOptions._RF+"/m/humanreselect/detail/main";
				var modal = $(this);
				var blockelm=modal.find('.modal-content');
				modal.find('.modal-content').html("");
				$.ajax({
					...
					modal.find('.modal-content').html(html);
					banknumberDetail.init(id,url,modal);
					setTimeout(function() {
						KTApp.unblock(blockelm);
					}, 3000);
				});
	...
	var initDetail= function(event,modal){
		...
		$.ajax({
			...
			modal.find('.modal-content').html(html);
			banknumberDetail.init(id,modal);
			setTimeout(function() {
				KTApp.unblock(blockelm);
			}, 3000);
		});
	...
	var initTable = function() {
		...
		table=tableCont.DataTable({
			...
			ajax: {
				"url": KTAppOptions._RF+"/m/humanreselect/list/list_holiday",
        	...
        	drawCallback: function(settings) {
				if(tableCont.find("tr.group").length<1){
                    ...
                    api.column(2, {page:'current'} ).data().each( function ( group, i ) {
                        if ( last !== group ) {
                            $(rows).eq( i ).before(
                                '<tr class="group"><td colspan="6" class="font-12"><strong>'+group+'</strong></td></tr>'
                            );
                            last = group;
                        }
                    });
	...
	var initForm= function(){
		...
		$('#detailSubModal1').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				var button = $(event.relatedTarget); 
				var id = button.data('id');
				var url = button.data('url')?button.data('url'):KTAppOptions._RF+"/m/department/detail/test";
				var modal = $(this);
				modal.find('.modal-content').html("");
				...
				$.ajax({
					...
					modal.find('.modal-content').html(html);
					banknumberFormSub.init(modal.find("form"),modal,0,0);
		...
		form.on('change', 'select[name="holiday[RefHolidayType]"]', function (event) {
			if($(this).val()==1){
				$("#type1").removeClass("d-none");
				$("#type1").find(".form-control").removeAttr("disabled");
				$("#type2").addClass("d-none");
				$("#type2").find(".form-control").attr("disabled","disabled");
			}else{
				$("#type2").removeClass("d-none");
				$("#type2").find(".form-control").removeAttr("disabled");
				$("#type1").addClass("d-none");
				$("#type1").find(".form-control").attr("disabled","disabled");
			}
		});
		form.ajaxForm({
			dataType:  'json',
				...
				if(typeof jsonData !== 'undefined' && jsonData!="" && jsonData._state) {
					...
					if(jsonData._refresh){
						if(jsonData._refreshfull){
							AppHoliday.getTable().ajax.reload();
						}else{
							AppHoliday.getTable().ajax.reload( null, false );
						}
					}
			...
			swal.fire({
				...
				if (result.value) {
					$.ajax({
						method: "POST",
						dataType:  'json',
						url: KTAppOptions._RF+"/process/humanreselect/removerefholiday",
						data: { 'holiday[id]': id},
						beforeSend: function( xhr ) {
		...
		$('#detailSubModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				$.ajax({
					...
					modal.find('.modal-content').html(html);
					banknumberFormSub.init(modal.find("form"),modal,id,paramid);
					banknumberFormSub.initRemove();
					setTimeout(function() {
						KTApp.unblock(blockelm);
					}, 3000);
				});
...
var banknumberFormSub = function() {
	var validator = {};
	var paramvalue = {};
	var form;
	var parentmodal;
	...
	var initForm= function(){
		...
		form.ajaxForm({
			dataType:  'json',
			type: 'post',
			data: {"holiday[id]":id,"paramid":paramid},
			error: function (data) {
				toastr.error("Алдаа гарсан байна. Err msg: "+data.responseText);
				submitBtn.html(submitBtnText);
				submitBtn.removeAttr("disabled");
