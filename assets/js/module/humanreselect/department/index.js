...
var AppDepartment = function() {
	...
	var init = function() {
		...
		$('#regModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				var url = button.data('url')?button.data('url'):KTAppOptions._RF+"/m/humanreselect/form/add_department";
				var modal = $(this);
				...
				$.ajax({
					...
					departmentForm.init(modal.find("form"),modal,id);
					departmentForm.initRemove();
					setTimeout(function() {
						KTApp.unblock(blockelm);
					}, 3000);
				});
		...
		$('#detailModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				$.ajax({
					...
					modal.find('.modal-content').html(html);
					departmentDetail.init(id,url,modal);
					setTimeout(function() {
						KTApp.unblock(blockelm);
					}, 3000);
				});
	...
	var initDetail= function(event,modal){
		...
		$.ajax({
			...
			modal.find('.modal-content').html(html);
			departmentDetail.init(id,modal);
			setTimeout(function() {
				KTApp.unblock(blockelm);
			}, 3000);
		});
	...
	var initTable = function() {
		...
		$(".status_change").click(function(){
			$("#search").find('select[name="searchdata[department_statusid]"]').val($(this).data('id'));
			var params = {};
		...
		table=tableCont.DataTable({
			...
			ajax: {
				"url": KTAppOptions._RF+"/m/humanreselect/list/list_department",
...
var departmentForm = function() {
	...
	var initForm= function(){
		form.on('change', 'input:radio[name="department[DepartmentHasParent]"]', function (event) {
			if($(this).val()==1){
				if($("#formIsControl").hasClass("d-none")){
					$("#formIsControl").removeClass("d-none");
					$("#formIsControl").find(".form-control").removeAttr("disabled");
				}
			}else{
				if(!$("#formIsControl").hasClass("d-none")){
					$("#formIsControl").addClass("d-none");
					$("#formIsControl").find(".form-control").attr("disabled","disabled");
				}
			}
		});
		$('#detailSubModal1').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				$.ajax({
					...
					modal.find('.modal-content').html(html);
					departmentFormSub.init(modal.find("form"),modal,0,0);
		...
		form.find('.kt-select2-custom').each(function() {
			var tmpid=$(this).attr("id");
			var placeholder=$(this).data("placeholder");
			form.find('#'+tmpid).select2({
			...
			swal.fire({
				...
				if (result.value) {
					$.ajax({
						method: "POST",
						dataType:  'json',
						url: KTAppOptions._RF+"/process/humanreselect/removedepartment",
						data: { 'department[id]': id},
						beforeSend: function( xhr ) {
		...
		$('#detailSubModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				$.ajax({
					...
					modal.find('.modal-content').html(html);
					departmentFormSub.init(modal.find("form"),modal,id,paramid);
					departmentFormSub.initRemove();
					setTimeout(function() {
						KTApp.unblock(blockelm);
					}, 3000);
				});
...
var departmentFormSub = function() {
	var validator = {};
	var paramvalue = {};
	var form;
	var parentmodal;
	...
	var initForm= function(){
		...
		form.ajaxForm({
			dataType:  'json',
			type: 'post',
			data: {"department[id]":id,"paramid":paramid},
			error: function (data) {
				toastr.error("Алдаа гарсан байна. Err msg: "+data.responseText);
				submitBtn.html(submitBtnText);
				submitBtn.removeAttr("disabled");
