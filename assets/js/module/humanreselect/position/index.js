...
var AppPosition = function() {
	...
	var init = function() {
		...
		$('#regModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				var url = button.data('url')?button.data('url'):KTAppOptions._RF+"/m/humanreselect/form/add_position";
				var modal = $(this);
	...
	var initTable = function() {
		...
		table=tableCont.DataTable({
			...
			ajax: {
				"url": KTAppOptions._RF+"/m/humanreselect/list/list_position",
...
//			}
		...
		if(form.find("select[name='position[PositionDepartmentID]']").length>0){
			form.on('change', 'select[name="position[PositionDepartmentID]"]', function (event) {
				...
				$.ajax({
					method: "POST",
					url: KTAppOptions._RF+"/m/humanreselect/form/add_position_field",
			...
			swal.fire({
				...
				if (result.value) {
					$.ajax({
						method: "POST",
						dataType:  'json',
						url: KTAppOptions._RF+"/process/humanreselect/removeposition",
						data: { 'position[id]': id},
						beforeSend: function( xhr ) {
