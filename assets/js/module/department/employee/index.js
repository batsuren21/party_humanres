	...
	var init = function() {
		...
		$('#regModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				var url = button.data('url')?button.data('url'):KTAppOptions._RF+"/m/department/form/add_employee";
				var modal = $(this);
		...
		$(document).on('click', '.detailsubpage', function (event) {
			var button=$(this);
			var url = button.data('url')?button.data('url'):KTAppOptions._RF+"/m/department/employee/main";
	...
	var initTable = function() {
		...
		$(document).on("change","#changeEmployeeState",function(){
			...
			$.ajax({
				method: "POST",
				dataType:  'json',
				url: KTAppOptions._RF+"/process/department/editperson",
		...
		if(tableCont.length>0){
			table=tableCont.DataTable({
				...
				ajax: {
					"url": KTAppOptions._RF+"/m/department/list/list_employee",
...
//			}
		...
		form.on('click', '#checkregister', function (event) {
			...
			var url=$(this).data("url")?KTAppOptions._RF+"/m/department/form/"+$(this).data("url"):KTAppOptions._RF+"/m/department/form/add_employee_field";
			$.ajax({
				method: "POST",
				url: url,
			...
			swal.fire({
				...
				if (result.value) {
					$.ajax({
						method: "POST",
						dataType:  'json',
						url: KTAppOptions._RF+"/process/department/removeemployee",
						data: { 'employee[id]': id},
						beforeSend: function( xhr ) {
	...
	var init = function() {
		...
		$('#detailPrintModalHumanres').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				$.ajax({
					method: "POST",
					url: KTAppOptions._RF+"/m/department/detail/print",
					data: { id:id,my:1},
					beforeSend: function( xhr ) {
						KTApp.block(blockelm, {
							overlayColor: '#ffffff',
	...
	var initForm= function(){
		...
		if(form.find("select[name='award[AwardRefID]']").length>0){
			form.on('change', 'select[name="award[AwardRefID]"]', function (event) {
				...
				$.ajax({
					method: "POST",
					url: KTAppOptions._RF+"/m/department/formemployee/award_field",
		...
		if(form.find("select[name='job[JobOrganTypeID]']").length>0){
			form.on('change', 'select[name="job[JobOrganTypeID]"]', function (event) {
				...
				$.ajax({
					method: "POST",
					url: KTAppOptions._RF+"/m/department/formemployee/job_field",
		...
		if(form.find("select[name='study[StudyDirectionID]']").length>0){
			form.on('change', 'select[name="study[StudyDirectionID]"]', function (event) {
				...
				$.ajax({
					method: "POST",
					url: KTAppOptions._RF+"/m/department/formemployee/study_field",
