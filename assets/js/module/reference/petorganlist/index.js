...
var AppOrganList = function() {
	...
	var init = function() {
		...
		$('#regModal').on('show.bs.modal', function (event) {
			if (event.namespace === 'bs.modal') {
				...
				var url = button.data('url')?button.data('url'):KTAppOptions._RF+"/m/reference/form/add_pet_organ";
				var modal = $(this);
	...
	var initTable = function() {
		...
		table=tableCont.DataTable({
			...
			ajax: {
				"url": KTAppOptions._RF+"/m/reference/list/list_pet_organ",
			...
			swal.fire({
				...
				if (result.value) {
					$.ajax({
						method: "POST",
						dataType:  'json',
						url: KTAppOptions._RF+"/process/reference/removepetorgan",
						data: { 'class[id]': id},
						beforeSend: function( xhr ) {
...
var organFormSub = function() {
	...
	var initForm= function(){
		...
		form.ajaxForm({
			dataType:  'json',
			type: 'post',
			data: {"class[id]":id,"paramid":paramid},
			error: function (data) {
				toastr.error("Алдаа гарсан байна. Err msg: "+data.responseText);
				submitBtn.html(submitBtnText);
				submitBtn.removeAttr("disabled");
