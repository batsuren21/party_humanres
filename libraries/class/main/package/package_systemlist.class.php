<?php
namespace Office;
class PackageSystemListClass extends \Office\CommonMain{
    use \OfficeTrait\Common;
    const TBL_PACKAGE_SYSTEMLIST="sys_packagesystemlist";
    ...
    static function getQueryCondition($search=array()){
        ...
        if(isset($search['packsyslist_id'])){
            $tmp_input=self::getParam($search['packsyslist_id']);
            $tmp_cond="";
    ...
    function getQueryBody($qry_cond=array(),$_type=1){
        ...
        $qry.=" from ".DB_DATABASE.".".self::TBL_PACKAGE_SYSTEMLIST." T";
        if(count($join_inner)>0) {
