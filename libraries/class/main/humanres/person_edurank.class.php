...
class PersonEduRankClass extends \Office\CommonMain{
    ...
    function validateAddRow($_data=array(),$type=1){
        ...
        if(!isset($_data["EduRankID"]) || $_data["EduRankID"]===""){
            $this->addError(\System\Error::ERROR_REQUIRED_EMPTY, 'Сонгоогүй байна',"edurank[EduRankID]");
        }
        if(!isset($_data["EduDate"]) || $_data["EduDate"]===""){
            $this->addError(\System\Error::ERROR_REQUIRED_EMPTY, 'Сонгоогүй байна',"edurank[EduDate]");
        }
        if(!isset($_data["EduLicence"]) || $_data["EduLicence"]===""){
            $this->addError(\System\Error::ERROR_REQUIRED_EMPTY, 'Хоосон байна',"edurank[EduLicence]");
        }
        if($type==1){
    ...
    function validateUpdateRow($_data=array(),$type=1){
        ...
        if(isset($_data["EduLicence"]) && $_data["EduLicence"]===""){
            $this->addError(\System\Error::ERROR_REQUIRED_EMPTY, 'Сонгоогүй байна',"edurank[EduLicence]");
        }
        if(isset($_data["EduDate"]) && $_data["EduDate"]===""){
            $this->addError(\System\Error::ERROR_REQUIRED_EMPTY, 'Хоосон байна',"edurank[EduDate]");
        }
        if($type==1){
