<?php
namespace Office;
class RefCountryClass extends \Office\CommonMain{
    use \OfficeTrait\Common;
    const TBL_COUNTRY="ref_country";
    ...
    function isExist(){
        return isset($this->_data["CountryID"]) && $this->_data["CountryID"]!=""?true:false;
    }
    static function getQueryCondition($search=array()){
        ...
        if(isset($search['country_get_table']) && $search['country_get_table']!=""){
            switch ($search['country_get_table']){
                case 1:
                    break;
            }
        }
        ...
        if(isset($search['country_id'])){
            if($search['country_id']==="") $search['country_id']=-1;
            elseif(is_array($search['country_id']) && count($search['country_id'])<1) $search['country_id']=[-1];
            
            $tmp_input=$search['country_id'];
            $tmp_cond="";
            if(is_array($tmp_input)){
                $tmp_cond=$_mainTable.".CountryID IN (".implode(",", $tmp_input).")";
            }elseif($tmp_input!==""){
                $tmp_cond=$_mainTable.".CountryID ='".$tmp_input."'";
            }
            if($tmp_cond!=""){
    ...
    function getQueryBody($qry_cond=array(),$_type=1){
        ...
        $qry.=" from ".DB_DATABASE.".".self::TBL_COUNTRY." T";
        if(count($join_inner)>0) {
    ...
    function getRowCount($search=array()){
        ...
        $qry = "select COUNT(T.CountryID) as AllCount"; 
