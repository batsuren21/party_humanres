<?php
    ...
    $now=new \DateTime();
    $search=array_merge($searchdata,['employee_get_table'=>6,'position_typeid'=>[1,2,3,4]]);
    if(!isset($searchdata['employee_isactive'])){
        $search=array_merge($search,['employee_isactive'=>1]);
    }
    $mainObj=new \Humanres\EmployeeClass();
    ...
    foreach ($_resultList as $j => $row){
        ...
        $btn='<a href="javascript:;" class="btn btn-primary btn-elevate btn-icon btn-sm" title="Дэлгэрэнгүй" data-id="'.$employeeObj->PersonID.'" data-toggle="modal" data-target="#detailModal" data-url="'.RF.'/m/humanreselect/detailemployee/main">
