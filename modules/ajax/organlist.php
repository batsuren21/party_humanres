...
$_resultList=$mainObj->getRowList(array_merge($search,array("orderby"=>array("OrganName"),"rowstart"=>(($page-1)*$length),"rowlength"=>$length)));

foreach ($_resultList as $j => $row){
    $organObj= \Office\OrganListClass::getInstance($row);
    $data[]=['id'=>$organObj->OrganID,'text'=>$organObj->OrganName];
}
