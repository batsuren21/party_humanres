<?php
    ...
    $now=new \DateTime();
    $search=$searchdata;
    $mainObj=new \Office\RefDirectionClass();
    ...
    $_mainList=$mainObj->getRowList(array_merge($search,array(
        "orderby"=>array("DirectionOrder"),"rowstart"=>$iDisplayStart,"rowlength"=>$iDisplayLength)));
    $data=array();
    
    foreach ($_mainList as $j => $row){
        $mainObj= \Office\RefDirectionClass::getInstance($row);
        $rownum= ($j +$iDisplayStart+ 1);
        $btn='<a href="javascript:;" class="btn btn-primary btn-elevate btn-icon btn-sm" title="Дэлгэрэнгүй" data-id="'.$mainObj->DirectionID.'" data-toggle="modal" data-target="#detailModal" data-url="'.RF.'/m/reference/detail/pet_direction">
        ...
        $data[]=array(
            ($rownum++),
            $btn,
            $mainObj->DirectionTitle,
            $mainObj->DirectionOrder,
            $mainObj->DirectionIsShow?"Тийм":"Үгүй",
        );
    }
