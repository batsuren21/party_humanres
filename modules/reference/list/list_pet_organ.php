<?php
    ...
    $now=new \DateTime();
    $search=$searchdata;
    $mainObj=new \Office\RefGovClassClass();
    ...
    $_mainList=$mainObj->getRowList(array_merge($search,array(
        "orderby"=>array("ClassOrder"),"rowstart"=>$iDisplayStart,"rowlength"=>$iDisplayLength)));
    $data=array();
    
    foreach ($_mainList as $j => $row){
        $mainObj= \Office\RefGovClassClass::getInstance($row);
        $rownum= ($j +$iDisplayStart+ 1);
        $btn='<a href="javascript:;" class="btn btn-primary btn-elevate btn-icon btn-sm" title="Дэлгэрэнгүй" data-id="'.$mainObj->ClassID.'" data-toggle="modal" data-target="#detailModal" data-url="'.RF.'/m/reference/detail/pet_organ_list">
