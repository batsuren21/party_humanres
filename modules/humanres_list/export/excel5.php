...
$array_header=array(
    ...
    array('title'=>"Цол","type"=>"Ref",'width'=>15),
    array('title'=>"Олгосон огноо","value"=>"EduDate",'width'=>15),
    array('title'=>"Гэрчилгээний дугаар","value"=>"EduLicence",'width'=>15),
);
...
$mainObj=new \Humanres\PersonEduRankClass();
$search=$searchdata;
...
foreach ($_resultList as $j => $row){
    ...
    $tmpMainObj= \Humanres\PersonEduRankClass::getInstance($row);
    $tmpObj= \Humanres\PersonClass::getInstance($row);
    
    $_refObj=\Humanres\ReferenceClass::getInstance(isset($_edurankList[$tmpMainObj->EduRankID])?$_edurankList[$tmpMainObj->EduRankID]:"");
        ...
        if(isset($array_header[$jj]['value'])){
            ...
            if(isset($array_header[$jj]['type'])){
                if($array_header[$jj]['type']=='Num'){
                    ...
                    $activesheet->setCellValueByColumnAndRow($jj, $i, $_refObj->RefRankTitle);
                }
            }
        }
    }
