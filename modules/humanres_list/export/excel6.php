...
$array_header=array(
    ...
    array('title'=>"Шагналын төрөл","type"=>"Ref",'width'=>30),
    array('title'=>"Шагналын дэд төрөл","type"=>"RefSub",'width'=>15,'is_str'=>1),
    array('title'=>"Байгууллагын нэр","value"=>"AwardOrganTitle",'width'=>20,'is_str'=>1),
    array('title'=>"Шагналын нэр","value"=>"AwardTitle",'width'=>20,'is_str'=>1),
    array('title'=>"Шийдвэрийн огноо","value"=>"AwardDate",'width'=>15),
    array('title'=>"Шийдвэрийн дугаар","value"=>"AwardLicence",'width'=>20),
);
...
foreach ($_resultList as $j => $row){
    ...
    for($jj=0; $jj<count($array_header); $jj++){
        if(isset($array_header[$jj]['value'])){
            $_str=$tmpObj->{$array_header[$jj]['value']};
            if($array_header[$jj]['value']=='AwardTitle' && $_str==""){
                $_str=html_entity_decode($_refSubObj->RefAwardTitle);
            }
            if(isset($array_header[$jj]['is_str']) && $array_header[$jj]['is_str']){
                $activesheet->setCellValueByColumnAndRow($jj, $i,html_entity_decode($_str));
            }else{
                $activesheet->setCellValueByColumnAndRow($jj, $i,$_str);
            }
        }else{
            if(isset($array_header[$jj]['type'])){
                if($array_header[$jj]['type']=='Num'){
                    ...
                    $activesheet->setCellValueByColumnAndRow($jj, $i, html_entity_decode($_refObj->RefAwardTitle));
                }elseif($array_header[$jj]['type']=='RefSub'){
                    $activesheet->setCellValueByColumnAndRow($jj, $i, html_entity_decode($_refSubObj->RefAwardTitle));
                }
            }
        }
    }
