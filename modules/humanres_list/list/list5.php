<?php
    ...
    $iTotalRecords = $mainObj->getRowCount(array_merge(["edurank_get_table"=>1,"person_get_table"=>1],$search));
    ...
    foreach ($_resultList as $j => $row){
        $mainObj= \Humanres\PersonEduRankClass::getInstance($row);
        $tmpObj= \Humanres\PersonClass::getInstance($row);
        
        $_refObj=\Humanres\ReferenceClass::getInstance(isset($_edurankList[$mainObj->EduRankID])?$_edurankList[$mainObj->EduRankID]:"");
        
        $rownum= ($j +$iDisplayStart+ 1);
        $btn='<a href="javascript:;" class="btn btn-primary btn-elevate btn-icon btn-sm" title="Дэлгэрэнгүй" data-url="'.RF.'/m/humanres/detailemployee/main?_spage=edurank" data-id="'.$tmpObj->PersonID.'" data-toggle="modal" data-target="#detailModalFelony">
